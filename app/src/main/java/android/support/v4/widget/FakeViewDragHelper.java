...
/**
 * FakeViewDragHelper is a utility class for writing custom ViewGroups. It offers a number
...
public class FakeViewDragHelper {
    ...
    private final ViewDelegate mFakeViewDelegate;

    /**
     * A Callback is used as a communication channel with the FakeViewDragHelper back to the
     * parent view using it. <code>on*</code>methods are invoked on siginficant events and several
     * accessor methods are expected to provide the FakeViewDragHelper with more information
    ...
    public abstract static class Callback {
         ...
         * one of these methods, the FakeViewDragHelper will enter {@link #STATE_SETTLING}
         * and the view capture will not fully end until it comes to a complete stop.
         * If neither of these methods is invoked before <code>onViewReleased</code> returns,
         * the view will stop in place and the FakeViewDragHelper will return to
         * {@link #STATE_IDLE}.</p>
         *
         ...
         * <p>
         * <p>FakeViewDragHelper may call this method multiple times for the same view even if
    ...
    /**
     * Factory method to create a new FakeViewDragHelper.
     *
    ...
    /**
     * Apps should use FakeViewDragHelper.create() to get a new instance.
    ...
    private FakeViewDragHelper(Context context,
        ...
        mFakeViewDelegate = viewDelegate;
    ...
    public void captureChildView(View childView, int activePointerId) {
        if (childView.getParent() != mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant "
                    + "of the FakeViewDragHelper's tracked parent view (" + mParentView + ")");
        }
    ...
    private boolean forceSettleCapturedViewAt(int finalLeft, int finalTop, int xvel, int yvel) {
        final int startLeft = getViewLeft(mCapturedView);
        final int startTop = getViewTop(mCapturedView);
    ...
    private int computeAxisDuration(int delta, int velocity, int motionRange) {
        ...
        final int width = getViewWidth(mParentView);
    ...
    public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
        ...
        mScroller.fling(getViewLeft(mCapturedView), getViewTop(mCapturedView),
                (int) VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId),
                (int) VelocityTrackerCompat.getYVelocity(mVelocityTracker, mActivePointerId),
    ...
    public boolean continueSettling(boolean deferCallbacks) {
        if (mDragState == STATE_SETTLING) {
            ...
            final int dx = x - getViewLeft(mCapturedView);
            final int dy = y - getViewTop(mCapturedView);

            if (dx != 0) {
                setViewOffsetLeftAndRight(mCapturedView, dx);
            }
            if (dy != 0) {
                setViewOffsetTopAndBottom(mCapturedView, dy);
            }
    ...
    protected boolean canScroll(View v, boolean checkV, int dx, int dy, int x, int y) {
        if (v instanceof ViewGroup) {
            ...
            final int scrollX = getViewScrollX(v);
            final int scrollY = getViewScrollY(v);
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                if (x + scrollX >= getViewLeft(child) && x + scrollX < getViewRight(child)
                        && y + scrollY >= getViewTop(child) && y + scrollY < getViewBottom(child)
                        && canScroll(child, true, dx, dy, x + scrollX - getViewLeft(child),
                        y + scrollY - getViewTop(child))) {
                    return true;
                }
            }
        }

        return checkV && (canViewScrollHorizontally(v, -dx)
                || canViewScrollVertically(v, -dy));
    }
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEventCompat.ACTION_POINTER_DOWN: {
                ...
                // A FakeViewDragHelper can only manipulate one view at a time.
                if (mDragState == STATE_IDLE) {
                    final int edgesTouched = mInitialEdgesTouched[pointerId];
                    if ((edgesTouched & mTrackingEdges) != 0) {
                        mCallback.onEdgeTouched(edgesTouched & mTrackingEdges, pointerId);
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                for (int i = 0; i < pointerCount; i++) {
                    ...
                    if (pastSlop) {
                        ...
                        final int oldLeft = getViewLeft(toCapture);
                        final int targetLeft = oldLeft + (int) dx;
                        final int newLeft = mCallback.clampViewPositionHorizontal(toCapture,
                                targetLeft, (int) dx);
                        final int oldTop = getViewTop(toCapture);
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDragState == STATE_DRAGGING) {
                    ...
                    dragTo(getViewLeft(mCapturedView) + idx, getViewTop(mCapturedView) + idy, idx,
                            idy);
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        int clampedX = left;
        int clampedY = top;
        final int oldLeft = getViewLeft(mCapturedView);
        final int oldTop = getViewTop(mCapturedView);
        if (dx != 0) {
            clampedX = mCallback.clampViewPositionHorizontal(mCapturedView, left, dx);
            setViewOffsetLeftAndRight(mCapturedView, clampedX - oldLeft);
        }
        if (dy != 0) {
            clampedY = mCallback.clampViewPositionVertical(mCapturedView, top, dy);
            setViewOffsetTopAndBottom(mCapturedView, clampedY - oldTop);
        }
    ...
    public boolean isViewUnder(View view, int x, int y) {
        ...
        return x >= getViewLeft(view)
                && x < getViewRight(view)
                && y >= getViewTop(view)
                && y < getViewBottom(view);
    }
    ...
    public View findTopChildUnder(int x, int y) {
        ...
        for (int i = childCount - 1; i >= 0; i--) {
            ...
            if (x >= getViewLeft(child) && x < getViewRight(child)
                    && y >= getViewTop(child) && y < getViewBottom(child)) {
                return child;
            }
        }
        return null;
    ...
    private int getEdgesTouched(int x, int y) {
        ...
        if (x < getViewLeft(mParentView) + mEdgeSize)
            result |= EDGE_LEFT;
        if (y < getViewTop(mParentView) + mEdgeSize)
            result |= EDGE_TOP;
        if (x > getViewRight(mParentView) - mEdgeSize)
            result |= EDGE_RIGHT;
        if (y > getViewBottom(mParentView) - mEdgeSize)
            result |= EDGE_BOTTOM;
    ...
    private boolean isValidPointerForActionMove(int pointerId) {
        if (!isPointerDown(pointerId)) {
            Log.e(TAG, "Ignoring pointerId=" + pointerId + " because ACTION_DOWN was not received "
                    ...
                    + " FakeViewDragHelper did not receive all the events in the event stream.");
            return false;
        }
        return true;
    }
